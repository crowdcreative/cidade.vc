<?php
	...
	if($post_id){
		...
		$servicosSelecionados = $_POST['servicos']; // Pega os dias marcados da semana que o evento acontece

		if(isset($servicosSelecionados)){
			sort($servicosSelecionados); // Coloca os dias em ordem
		}
		
		$old = get_post_meta($post_id, 'servicos_oferecidos', true); 


		if ($servicosSelecionados && $servicosSelecionados != $old) {
			update_post_meta($post_id, 'servicos_oferecidos', $servicosSelecionados);
		} elseif ('' == $servicosSelecionados && $old) {
			delete_post_meta($post_id, 'servicos_oferecidos', $old);
		}
