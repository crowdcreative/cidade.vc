	...
	$(document).ready(function(){
	    ...
	    $('#titulo').focusout(function(){
	    	...
	    	$.ajax({
				url: '<?php echo $localiza_url; ?>/controllers/classes/verificador-de-inputs-classe.php',
				type: 'POST',
				dataType: 'json',
				...
				data: {
					'conteudo': $(this).val(),
					'tipo': $(this).attr('tipo'),
					'tamanhoMin': $(this).attr('tamanhoMin'),
					'tamanhoMax': $(this).attr('tamanhoMax')
				},
				success: function(resposta){

					if(resposta.tipo == 'true' && resposta.tamanhoMax == 'true' && resposta.tamanhoMin == 'true'){

						input.parents('.bloco').addClass('has-success').removeClass('has-error');
						input.parents('.bloco').find('.input-glyphicon').addClass('glyphicon glyphicon-ok-sign').removeClass('glyphicon-exclamation-sign');
						input.parents('.bloco').find('.input-glyphicon').css('color', '#468847');
						input.parents('.bloco').find('.help-block').show();
						input.parents('.bloco').find('.error-block').hide();

					}

					if(resposta.tamanhoMin == 'false'){

						input.parents('.bloco').addClass('has-error').removeClass('has-success');
						input.parents('.bloco').find('.input-glyphicon').addClass('glyphicon glyphicon-exclamation-sign').removeClass('glyphicon-ok-sign');
						input.parents('.bloco').find('.input-glyphicon').css('color', '#B94A48');
						input.parents('.bloco').find('.help-block').hide();
						input.parents('.bloco').find('.error-block').fadeIn().text('O texto digitado é muito pequeno, tente um maior. =)');

					}

					if(resposta.tamanhoMax == 'false'){

						input.parents('.bloco').addClass('has-error').removeClass('has-success');
						input.parents('.bloco').find('.input-glyphicon').addClass('glyphicon glyphicon-exclamation-sign').removeClass('glyphicon-ok-sign');
						input.parents('.bloco').find('.input-glyphicon').css('color', '#B94A48');
						input.parents('.bloco').find('.help-block').hide();
						input.parents('.bloco').find('.error-block').fadeIn().text('O texto digitado é muito longo, tente um menor =)');
